...
while 1:
    try:
        game_mode = int(input('\nΔώστε τρόπο παιχνιδιού Κλασικός (1), Νέος (2): '))
...
while(open_cards != total_cards):
    ...
    if cards[first_pos][0] == cards[sec_pos][0]: # Σημαίνει ότι έχουν το ίδιο σύμβολο.
        ...
        if cards[first_pos][0] == 'J':
            play_id = play_id # Παίζει ο ίδιος πάλι (δεν αυξάνεται το play_id)
        elif cards[first_pos][0] == 'K':
        ...
        elif game_mode == 2: # Σημαίνει ότι δεν παίζει με τον κλασικό τρόπο.
            if cards[first_pos][1] == cards[sec_pos][1]: # Ελέγχουμε αν είναι ίδια σειρά (π.χ. σπαθί)
                points = cards[first_pos][2] + cards[sec_pos][2]
                add_points(total_points, play_id, points)
                open_cards = open_cards+2
        else:
            print('Ανεπιτυχές ταίριασμα!')
